...
class ConvGRU(nn.Module):
    ...
    def __init__(
        self, input_channels: int, hidden_channels: int, num_layers: int = 2, kernel_size: int = 3
    ):
        super().__init__()
    ...
    def forward(
        self, x: torch.Tensor, h_prev: list[torch.Tensor] = None
...
class HybridTerrainLoss(nn.Module):
    ...
    def __init__(self, w_sparse: float = 20.0, w_occluded: float = 1.0, w_grad: float = 10.0):
        super().__init__()
    ...
    def forward(
        ...
        total_loss = (
            self.w_sparse * loss_sparse + self.w_occluded * loss_occluded + self.w_grad * loss_grad
        )

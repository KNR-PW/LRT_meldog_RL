...
from .projector import euler_from_quat  # noqa: F401
...
def transform_height_map_with_mask(
    ...
    transformed_map = F.grid_sample(
        prev_map, grid_transformed, mode="bilinear", padding_mode="border", align_corners=True
    )

    # Sample valid mask with zeros padding (out of bounds = invalid)
    transformed_valid = F.grid_sample(
        prev_valid, grid_transformed, mode="bilinear", padding_mode="zeros", align_corners=True
    )
    # Threshold to binary
    ...
    def forward_sequence(
        ...
        for t in range(T):
            ...
            pred = self.forward(
                sparse_seq[t],
                mask_seq[t],
                prev_output_transformed,
                prev_valid_transformed,
                grav_seq[t],
            )
            preds.append(pred)

            # Update for next timestep
            use_teacher = target_seq is not None and torch.rand(1).item() < teacher_forcing_ratio

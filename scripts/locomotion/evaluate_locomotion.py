...
parser.add_argument(
    "--task", type=str, default="Meldog-RL-Locomotion-Rough-Sim-v0", help="Task name."
)
...
parser.add_argument(
    ...
    type=str,
    default="task",
    choices=["task", "flat", "rough", "obs", "rough_obs"],
    help=(
...
def fall_force_bw() -> float:
    ...
    return (
        FALL_BASE_FORCE_BW if args_cli.fall_base_force_bw is None else args_cli.fall_base_force_bw
    )
...
def main():
    ...
    adapter.configure_cfg(
        env_cfg,
        num_envs=args_cli.num_envs,
        seed=args_cli.seed,
        device=device,
        enable_cameras=USER_ENABLE_CAMERAS,
        benchmark=args_cli.benchmark,
    )
    ...
    if args_cli.benchmark and args_cli.bench_terrain not in ("task", "flat"):
        ...
        print(
            f"[BENCH] {len({(c[0], c[1]) for c in cells})} distinct terrain cells over "
    ...
    if args_cli.video:
        ...
        print(
            f"[INFO] Video: envs {view_envs} ({', '.join(kinds)}), "
    ...
    if args_cli.benchmark and args_cli.bench_commands == "froude":
        ...
        cmd_scale = (k, 1.0 / k)
        print(
            f"[BENCH] froude commands: leg {leg_length:.3f} m -> linear x{k:.2f}, angular x{1 / k:.2f}"
        )
    ...
    if args_cli.benchmark and args_cli.profile == "real":
        ...
        print(
            f"[BENCH] motor strength x{REAL_MOTOR_STRENGTH[0]}-{REAL_MOTOR_STRENGTH[1]} per env, "
    ...
    if args_cli.record:
        ...
        if leg_joint_ids is None:
            print(
                "[WARN] joint names do not group per leg; the analyzer skips the asymmetry metrics."
            )
        ...
        if not has_posture:
            print(
                "[WARN] env has no height scanner; "
    ...
    if args_cli.benchmark and args_cli.profile == "real":
        delayed_actions = torch.zeros(
            (args_cli.num_envs, raw_env.action_space.shape[-1]), device=device
        )
    ...
    if not base_ids:
        print(
            f"[WARN] base body '{adapter.spec.base_body}' not in contact sensor; fall rule uses tilt only."
        )
    if args_cli.full_episodes:
    ...
    if args_cli.benchmark:
        print(
            f"[INFO] Benchmark mode ON (seed={args_cli.seed}): scripted commands "
    ...
    with torch.inference_mode():
        while total_episodes < args_cli.num_episodes:
            ...
            if args_cli.record:
                ...
                rec["foot_pos_w"].append(robot.data.body_pos_w[:, feet_robot_ids].cpu().numpy())
                rec["foot_vel_w"].append(robot.data.body_lin_vel_w[:, feet_robot_ids].cpu().numpy())
                ...
                if has_terrain_levels:
                    rec["terrain_levels"].append(
                        terrain.terrain_levels.cpu().numpy().astype(np.int16)
                    )
                if has_terrain_types:
                    rec["terrain_types"].append(
                        terrain.terrain_types.cpu().numpy().astype(np.int16)
                    )
                if has_posture:
            ...
            if torch.any(dones):
                ...
                for idx in done_indices:
                    ...
                    first_survived[idx] = bool(time_outs[idx].item()) and not bool(
                        first_fell[idx].item()
                    )
                    is_timeout = (
                        first_survived[idx].item() if args_cli.benchmark else time_outs[idx].item()
                    )
                    if is_timeout:
                        success_count += 1
                    else:
            ...
            if total_episodes % 50 == 0 and total_episodes > 0:
                print(
                    f"Progress: {total_episodes}/{args_cli.num_episodes} | "
                    f"Success: {success_count / total_episodes * 100:.1f}%",
                    end="\r",
                )
...
def _write_rollout(save_dir, rec, raw_env, adapter, foot_names_canonical, first):
    ...
    with h5py.File(rollout_path, "w") as f:
        ...
        if rec["terrain_levels"]:
            f.create_dataset(
                "terrain_levels",
                data=np.stack(rec["terrain_levels"], axis=0).astype(np.int16),
                **gzip,
            )
        if rec["terrain_types"]:
            f.create_dataset(
                "terrain_types",
                data=np.stack(rec["terrain_types"], axis=0).astype(np.int16),
                **gzip,
            )
        ...
        f.create_dataset(
            "knee_joint_idx", data=np.asarray(adapter.knee_joint_ids(), dtype=np.int32)
        )
        if first["leg_joint_ids"] is not None:
            # (4, joints per leg) in FL, FR, RL, RR order -> asymmetry metrics in the analyzer.
            f.create_dataset(
                "leg_joint_idx", data=np.asarray(first["leg_joint_ids"], dtype=np.int32)
            )
        ...
        if args_cli.full_episodes:
            f.create_dataset(
                "first_episode_fell", data=first["fell"].cpu().numpy().astype(np.uint8)
            )
            f.create_dataset(
                "first_episode_survived", data=first["survived"].cpu().numpy().astype(np.uint8)
            )
            f.create_dataset(
                "first_episode_length", data=first["length"].cpu().numpy().astype(np.int32)
            )
        if first["cells"] is not None:
            f.create_dataset(
                "terrain_cell", data=np.asarray([c[:2] for c in first["cells"]], dtype=np.int16)
            )

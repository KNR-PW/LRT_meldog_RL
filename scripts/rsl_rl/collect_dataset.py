"""
Dataset Collector for Meldog (Merged V3)
- Features: 4x Depth + Raycast GT + Robot Pose (Pos/Quat).
- Visualization: Restored 'preview.mp4' (Top RGB + Depth overlays).
"""
...
def main():
    ...
    clip_val = agent_cfg.get("clip_actions", 1.0) if isinstance(agent_cfg, dict) else getattr(agent_cfg, "clip_actions", 1.0)
    ...
    timestamp = datetime.now().strftime("PD_v0_%Y-%m-%d_%H-%M-%S")
    save_dir = os.path.join("datasets", timestamp)
    os.makedirs(save_dir, exist_ok=True)
    h5_path = os.path.join(save_dir, "dataset.h5")
    
    print(f"[INFO] Saving data to: {save_dir}")

    # [UPDATED] Buffers include pose
    buffers = [
        {
    ...
    with h5py.File(h5_path, 'w') as h5file:
        def save_episode_to_h5(env_idx, force_save=False):
            ...
            for key in buffers[env_idx]: buffers[env_idx][key] = []
        ...
        while simulation_app.is_running():
            ...
            # [RESTORED] RGB Top (Only for video, not saving to H5)
            try:
                rgb_top = raw_env._tiled_camera_top.data.output["rgb"].clone()
            except:
                rgb_top = None # Handle missing camera gracefully

            # 2. Get GT Height
            if hasattr(raw_env, "_gt_scanner") and raw_env._gt_scanner:
                # Use trunk_z to get relative height

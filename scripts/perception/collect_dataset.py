...
parser.add_argument(
    "--min_episode_len", type=int, default=50, help="Discard episodes shorter than this."
)
...
def process_raycast(tensor_scan, label="Raycast", target_h=240, target_w=424):
    ...
    final_img = cv2.copyMakeBorder(
        square_img, 0, 0, pad_l, pad_r, cv2.BORDER_CONSTANT, value=[0, 0, 0]
    )
    cv2.putText(final_img, label, (pad_l + 10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 0), 3)
    cv2.putText(
        final_img, label, (pad_l + 10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 1
    )
    return final_img
...
def main():
    ...
    video_writer = cv2.VideoWriter(
        str(video_path), cv2.VideoWriter_fourcc(*"mp4v"), 20, (1272, 480)
    )
    video_frames = 0
    ...
    with h5py.File(h5_path, "w") as h5file:
        ...
        def save_episode_to_h5(env_idx, force_save=False):
            ...
            if ep_len >= args_cli.min_episode_len or (force_save and ep_len > 0):
                ...
                grp.create_dataset(
                    "depth_front",
                    data=to_uint16_mm(buffers[env_idx]["depth_front"]),
                    compression="gzip",
                )
                grp.create_dataset(
                    "depth_rear",
                    data=to_uint16_mm(buffers[env_idx]["depth_rear"]),
                    compression="gzip",
                )
                grp.create_dataset(
                    "depth_left",
                    data=to_uint16_mm(buffers[env_idx]["depth_left"]),
                    compression="gzip",
                )
                grp.create_dataset(
                    "depth_right",
                    data=to_uint16_mm(buffers[env_idx]["depth_right"]),
                    compression="gzip",
                )
                grp.create_dataset(
                    "gt_height", data=to_int16_mm(buffers[env_idx]["gt_height"]), compression="gzip"
                )
                grp.create_dataset(
                    "robot_pos", data=np.array(buffers[env_idx]["robot_pos"], dtype=np.float32)
                )
                grp.create_dataset(
                    "robot_quat", data=np.array(buffers[env_idx]["robot_quat"], dtype=np.float32)
                )
        ...
        if raw_env._cameras.get("front") is None:
            print(
                "[ERROR] Cameras not configured! Use a Dataset task (e.g., Meldog-RL-Dataset-Rough-v0)"
            )
        ...
        while True:
            ...
            if step_count % 500 == 0:
                print(
                    f"[INFO] Progress: {step_count}/{args_cli.max_steps} steps, {episode_counter[0]} episodes saved"
                )

...
def train_v5(args):
    ...
    criterion = HybridTerrainLoss(
        w_sparse=args.w_sparse, w_occluded=args.w_occluded, w_grad=args.w_grad
    )
    ...
    for epoch in range(args.epochs):
        ...
        print(
            f"Epoch {epoch+1} | Val L1 Obs: {val_obs:.2f}cm | Val L1 Occ: {val_occ:.2f}cm | LR: {scheduler.get_last_lr()[0]:.2e}"
        )
...
def train_v6(args):
    ...
    for epoch in range(args.epochs):
        ...
        print(
            f"Epoch {epoch+1} | Val L1 Obs: {val_obs:.2f}cm | Val L1 Occ: {val_occ:.2f}cm | TF: {tf_ratio:.2f}"
        )
